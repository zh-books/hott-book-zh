...
然而，我们可以针对该方法的特定变体陈述引理。这些引理的证明几乎是平凡的；主要目的是通过一般性陈述来澄清该方法。最简单的情况是使用编码-解码方法来描述类型的循环空间，如 \cref{thm:path-coprod} 和 \cref{cor:omega-s1} 中。

\begin{lem}[循环空间的编码-解码方法 (Encode-decode for Loop Spaces)]\label{lem:encode-decode-loop}
\index{循环空间 (loop space)}%
给定一个带基点的类型 $(A,a_0)$ 和一个纤维丛
$\code : A \to \type$，如果
\begin{enumerate}
  \item $c_0 : \code(a_0)$,\label{item:ed1}
  \item $\decode : \prd{x:A} \code(x) \to (\id{a_0}{x})$,\label{item:ed2}
  \item 对于所有 $c : \code(a_0)$, \id{\transfib{\code}{\decode(c)}{c_0}}{c}，并且\label{item:ed3}
...
\begin{lem}[循环空间截断的编码-解码 (Encode-decode for Truncations of Loop Spaces)]
...
\begin{thm}
  \index{球的同伦群 (homotopy group of sphere)}
...
\begin{thm}[Blakers--Massey 定理 (Blakers--Massey theorem)]\label{Blakers-Massey}
\indexdef{定理!Blakers--Massey (theorem!Blakers--Massey)}%
\indexsee{Blakers--Massey 定理}{定理, Blakers--Massey (theorem, Blakers--Massey)}%
...
\begin{thm}[Eilenberg--Mac Lane 空间 (Eilenberg--Mac Lane Spaces)]\label{Eilenberg-Mac-Lane-Spaces}
\index{Eilenberg--Mac Lane 空间 (Eilenberg--Mac Lane space)}
对于任意阿贝尔群 (abelian group) $G$ 和正整数 $n$，存在一个 $n$-类型
$K(G,n)$，使得 $\pi_n(K(G,n)) = G$，且  $\pi_k(K(G,n)) = 0$
对于 $k\neq n$。
\end{thm}

\begin{thm}[覆盖空间 (Covering spaces)]\label{thm:covering-spaces}
\index{覆盖空间 (covering space)}%
...
\index{可判定!等价性 (decidable!equality)}%
...
根据 \cref{sec:hopf} 中 Hopf 纤维丛的例子，定义 \textbf{Junior Hopf 纤维丛 (junior Hopf fibration)} 作为 $\Sn ^1$ 上的纤维丛（即一个类型族），其基点上的纤维为 $\Sn ^0$，其总空间为 $\Sn ^1$。这也被称为圆 $\Sn ^1$ 的“扭曲双重覆盖” (twisted double cover)。
\end{ex}

\begin{ex}\label{ex:SuperHopf}
再次参考 \cref{sec:hopf} 中 Hopf 纤维丛的例子，定义一个类似的纤维丛，其基点上的纤维为 $\Sn ^3$，其总空间为 $\Sn ^7$。这是同伦类型论中的一个开放问题（在经典同伦理论中，已知存在这样的纤维丛）。
\end{ex}

\begin{ex}\label{ex:vksusppt}
继续 \cref{eg:suspension} 的内容，证明如果 $A$ 有一个点 $a:A$，那么我们可以将 $\pi_1(\susp A)$ 与生成元为 $\trunc0A$，关系为 $\tproj0a = e$ 的群同一化。
然后显示如果我们假设排中律，这也就是 $\trunc0 A \setminus \{\tproj0 a \}$ 上的自由群 (free group)。
\end{ex}

\begin{ex}\label{ex:vksuspnopt}
继续 \cref{eg:suspension} 的内容，但这次不假设 $A$ 是带基点的，显示我们可以将 $\pi_1(\susp A)$ 与生成元 $\trunc0A \times \trunc0A$ 和关系
\begin{equation*}
(a,b) = \opp{(b,a)},
\qquad
(a,c) = (a,b)\cdot (b,c),
